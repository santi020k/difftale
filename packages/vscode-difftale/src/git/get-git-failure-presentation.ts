...
const removeAnsiSequences = (value: string): string => value
  .split(ANSI_ESCAPE_CHARACTER)
  .map((segment, index) => index === 0 ? segment : segment.replace(ANSI_SEQUENCE_SUFFIX_PATTERN, ''))
  .join('')

const getOutputLines = (output: string): string[] => removeAnsiSequences(output)
  .split(/\r?\n/u)
  .map(line => line.trim())
  .filter(Boolean)
...
const getSummary = (lines: readonly string[], exitCode: number | null): string => {
  ...
  const meaningfulLine = lines.find(
    line => !COMMAND_OUTPUT_PATTERN.test(line) &&
